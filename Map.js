...
function Node(x, y, t) {
	...
	this.neighbours = function(goal) {
		...
		for(var i=0;i<(allowDiagonals?8:4);++i) {
			if(this.x+dir[i][0] < 0 || this.x+dir[i][0] > 49) continue;
			if(this.y+dir[i][1] < 0 || this.y+dir[i][1] > 49) continue;
...
function pathFind(x, y, x2, y2) {
	...
	while(open.length > 0) {
		...
		for(var i=0,c=neighbours.length;i<c;++i) {
			...
			//if(V_score[next.coord]) continue;
			...
			if(isBetter) {
				...
				f_score[next.coord] = g_score[next.coord] + h(next, goal) +map[next.x][next.y].weight;
				
				closed[node] = false;
				map[next.x][next.y].weight += 1;
				...
				// drawing
				var s = Math.floor(g_score[next.coord]*10);
